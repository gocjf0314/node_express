...
request.post(clientCredentialOptions, function (error: Error, response, body) {
...
app.get("/callback", async function (req: Request, res: Response) {
    ...
    if (state === null) {
        ...
        request.post(
            authOptions,
            async function (error: Error, response: request.Response, body) {
    ...
    request.post(
        ...
        function (error: Error, response: request.Response) {
            ...
            if (!error && response.statusCode === 200) {
                ...
                console.log(`Fail to get refresh_token: ${error}`);
            }
        }
    );
});
